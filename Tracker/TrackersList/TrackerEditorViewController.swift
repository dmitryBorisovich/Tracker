...
final class TrackerEditorViewController: UIViewController {
    ...
    weak var delegate: TrackerEditingDelegate?
    ...
    private func insertData() {
        ...
        if let selectedCategory = selectedCategory {
            didSelectCategory(name: selectedCategory)
        }
        ...
        indicateSelectedEmoji()
        indicateSelectedColor()
        
        updateCreateButtonState()
    }
    
    private func indicateSelectedEmoji() {
        guard let selectedEmoji = selectedEmoji else { return }
        if let emojiIndex = EmojiAndColor.shared.emojiArray.firstIndex(of: selectedEmoji) {
            let emojiIndexPath = IndexPath(item: emojiIndex, section: 0)
            selectedEmojiIndex = emojiIndexPath
            DispatchQueue.main.async {
                self.trackerParamsCollection.selectItem(at: emojiIndexPath, animated: false, scrollPosition: [])
                let cell = self.trackerParamsCollection.cellForItem(at: emojiIndexPath) as? TrackerParamsCollectionViewCell
                cell?.didSelectCell()
            }
        }
    }
    
    private func indicateSelectedColor() {
        guard let selectedColor = selectedColor else { return }
        
        let colorMarshalling = UIColorMarshalling()
        let selectedColorHex = colorMarshalling.hexString(from: selectedColor)
        
        if let colorIndex = EmojiAndColor.shared.colorsArray.firstIndex(where: { color in
            guard let color else { return false }
            let colorHex = colorMarshalling.hexString(from: color)
            return colorHex == selectedColorHex
        }) {
            let colorIndexPath = IndexPath(item: colorIndex, section: 1)
            selectedColorIndex = colorIndexPath
            DispatchQueue.main.async {
                self.trackerParamsCollection.selectItem(at: colorIndexPath, animated: false, scrollPosition: [])
                if let cell = self.trackerParamsCollection.cellForItem(at: colorIndexPath) as? TrackerParamsCollectionViewCell {
                    cell.didSelectCell()
                }
            }
        } else {
    ...
    private func updateCreateButtonState() {
        ...
        saveButton.backgroundColor = isReady ? .tBlack : .tGray
    }
    ...
    @objc private func saveButtonPressed() {
        ...
        delegate?.didEditedTracker(tracker)
        if selectedCategory != initialCategory {
            delegate?.didChangeCategory(for: tracker.id, newCategory: trackerParams.category)
        }
...
extension TrackerEditorViewController: UICollectionViewDelegateFlowLayout {
    // Размер ячейки
    func collectionView(_ collectionView: UICollectionView,
...
extension TrackerEditorViewController: ScheduleViewControllerDelegate {
...
extension TrackerEditorViewController: CategoryViewControllerDelegate {
...
extension TrackerEditorViewController: TrackerParamsCollectionViewCellDelegate {
